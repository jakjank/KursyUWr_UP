# How?
# normal A* (full_cost = pred_cost + cur_cost)
# We prioritize comandos who are the furthest from the closest end position
...
def a_star(s_pos: set, e_pos: set, walls: list, dist_map: list):
    visited = {}
    pq = []
    heapq.heappush(pq, (0,0,s_pos,""))
    visited[frozenset(s_pos)] = 0

    while pq:
        #print("pq len",len(pq))
        h_cost, cost, state, path = heapq.heappop(pq)

        if finished(state, e_pos):
            return path
        
        for direction in directions:
            new_state = move(state, direction, walls)
            new_path = path + dir_to_char[direction]
            new_cost = cost + 1
            ns_hash =frozenset(new_state)

            if ns_hash not in visited or new_cost < visited[ns_hash]:
                visited[ns_hash] = new_cost
