...
# tylko zalogowani uzytkownicy beda mogli dodawac znajomych
def auhenticate(name, password):
...
def local_cache(func):
    def wrapper(*args, **kwargs):
        ...
        if len(argv) > 0:
            if argv[0] == "--dodaj-z":
                if not auhenticate(argv[1],argv[2]):
                    return "Brak dostepu"

        # print(argv)
        db = shelve.open("/home/kuba/Documents/studia/rozszerzony python/lista14/query.cache")
        if str(argv) in db and "--autorzy" not in argv and "--znajomi" not in argv and "--ksiazki" not in argv:
            # print("uzywamy kasza")
            res = db[str(argv)]
            return res
        # print("liczymy main")
...
def main() -> None:
    args = sys.argv
    if len(args) == 1:
        return "zobacz 'python ksiazki.py --help'"
    args = args[1:]
    if args[0] == '--help':
        return """
...
--dodaj-z <nazwa> <haslo> <id> <imie> <email>
...
"""
        ...
        if args[0] == '--autorzy-API':
            return server.autorzy_read()
        if args[0] == '--znajomi-API':
            return server.znajomi_read()
        if args[0] == '--ksiazki-API':
            return server.ksiazki_read()
