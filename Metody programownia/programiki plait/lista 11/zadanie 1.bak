...
(define (parse-exp [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{tryE ANY ANY} s)                    ;
     (tryE (parse-exp (second (s-exp->list s)))          ;
           (parse-exp (third (s-exp->list s))))]         ; TO DODANE
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    ...
    [(errorE l m)
     (err l m)]
    [(tryE e1 e2)                            ;
     (try (eval e1) (lambda () (eval e2)))]  ; TO DODANE
     ))

;Przyklady
;   (eval (parse-exp `{tryE (/ 1 0) 101}))
;   (eval (parse-exp `{tryE (/ 22 (= 1 1)) 101}))
;   (eval (parse-exp `{tryE (+ 1 2) 101}))
