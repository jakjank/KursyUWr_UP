...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda ANY ANY} s)
     (cond
       [(empty? (s-exp->list (second (s-exp->list s))))
        (parse (third (s-exp->list s)))]
       [else
        (lamE (s-exp->symbol (first (s-exp->list (second (s-exp->list s)))))
              (parse `(lambda ,(list->s-exp (rest (s-exp->list (second (s-exp->list s))))) ,(third (s-exp->list s)))))])]
    ...
    [(s-exp-match? `{ANY ANY} s)
     (cond
       ...
       [else
        (appE (parse `(,(first (s-exp->list s)) ,(list->s-exp (rest (s-exp->list (second (s-exp->list s)))))))
              (parse (first (s-exp->list (second (s-exp->list s))))))])]
    [else (error 'parse "invalid input")]))

; Przyklady
; (parse `{lambda (x y z) (+ z (- x y))})
; (parse `{lambda (x) (* x 3)})
; (eval (parse `((lambda (x) (+ x 1)) (3))) mt-env)
; (eval (parse `((lambda (x y) (+ x (* 2 y))) (3 4))) mt-env)
