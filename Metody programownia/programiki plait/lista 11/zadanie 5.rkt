...
;-----------------------------------------------------------------------------
(define (look-for-fv [e : Exp] [env : (Listof Symbol)]) : (Listof Symbol)
  (type-case Exp e
    [(numE n) '()]
    [(opE o l r) (append (look-for-fv l env) (look-for-fv r env))]
    [(ifE b l r)
     (append (look-for-fv b env) (append (look-for-fv l env) (look-for-fv r env)))]
    [(varE x)
     (if (in-env? x env)
         '()
         (list x))]
    [(letE x e1 e2)
     (append (look-for-fv e1 env) (look-for-fv e2 (cons x env)))]
    [(lamE x b)
     (look-for-fv b (cons x env))]
    [(appE e1 e2)
     (append (look-for-fv e1 env) (look-for-fv e2 env))]))

(define (in-env? [n : Symbol] [env : (Listof Symbol)]) : Boolean
  (type-case (Listof Symbol) env
    [empty #f]
    [(cons b rst-env) (cond
                        [(eq? n (first env))
                         #t]
                        [else (in-env? n rst-env)])]))

(define (fv e)
  (look-for-fv e '()))

; przyklad: (fv (parse `(let x 2 (+ y (let z 3 z)))))
;-------------------------------------------------------------------------------
