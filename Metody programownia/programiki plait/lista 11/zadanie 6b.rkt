...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda ANY ANY} s)
     (lamE (map (lambda (x) (s-exp->symbol x)) (s-exp->list (second (s-exp->list s))))
...
; (eval (parse `{{lambda (x y z) (+ x (/ y z))} (1 2 3)}) mt-env)
    ...
    [(lamE xs b)
     (cond
       [(empty? xs)
        (eval b env)]
       [else
        (funV (λ (v) (eval (lamE (rest xs) b) (extend-env env (first xs) v))))])]
    [(appE e1 e2s)
     (cond
       [(empty? e2s)
        (eval e1 env)]
       [else
        (apply (eval (appE e1 (rest e2s)) env) (eval (first e2s) env))])]))
