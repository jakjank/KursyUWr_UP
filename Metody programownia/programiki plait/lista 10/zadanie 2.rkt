...
(define (decompile-help [c : Code]) : (Exp * Code)
  (type-case Instr (first c)
    [(pushI n)
     (pair (numE n)
           (rest c))]
    [(opI o)
     (pair (opE o (fst (decompile-help (snd (decompile-help (rest c))))) (fst (decompile-help (rest c))))
           (snd (decompile-help (snd (decompile-help (rest c))))))]))
;          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
; Zwracamy kawałek listy na ktory jeszcze nigy nie patrzylismy

(define (decompile [c : Code]) : Exp
  (fst (decompile-help (reverse c))))

; Testy do decompile
(module+ test
  (test (decompile (compile (parse `1)))
        (numE 1))
  (test (decompile (compile (parse `{+ 1 2})))
        (opE (add) (numE 1) (numE 2)))
  (test (decompile (compile (parse `{- 1 {* 2 3}})))
        (opE (sub) (numE 1) (opE (mul) (numE 2) (numE 3))))
  (test (decompile (compile (parse `{/ {+ 1 2} 3})))
        (opE (div) (opE (add) (numE 1) (numE 2)) (numE 3)))
  (test (decompile (compile (parse `{+ 1 {- {* 2 3} 4}})))
        (opE (add) (numE 1) (opE (sub) (opE (mul) (numE 2) (numE 3)) (numE 4)))))
