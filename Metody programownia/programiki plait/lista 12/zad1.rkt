...
(define fix (parse `{ lambda { f }
{ let w { lambda { g } { f { lambda { z } {{ g g } z }}}}
{ w w }}}))

(define f_fact (parse `{ lambda { fact }
{ lambda { n }
{ if {= n 0}
1
{* n { fact {- n 1}}}}}}))
...
;Env bedzie trzymac symbole i Exp ktore bedziemy wyliczac w przypadku potrzeby
(define-type Binding
  (bind [name : Symbol]
        [val : Exp]))
...
;doatkowa funkcja zwracajaca srodowisko bez ewaluowanej zmiennej
;zeby sie nie zapetlac w przypadku przykrycia nazw
(define (env-without-var [env : Env] [x : Symbol]) : Env
  (type-case (Listof Binding) env
    [empty '()]
    [(cons b rst-env) (cond
                        [(eq? x (bind-name b))
                         rst-env]
                        [else
                         (cons b (env-without-var rst-env x))])]))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     ;(lookup-env x env)
     (eval (lookup-env x env) (env-without-var env x))]
    [(letE x e1 e2)
     ;(let ([v1 (eval e1 env)])
...
; przyklad: (eval (parse `((lambda (x) (+ 1 x)) (/ 2 1))) mt-env)
; jak zaprezentowac roznice?
