...
(define (applyA [v1 : Value] [v2 : Value]) : Value ;<-----HERE!
  (type-case Value v1
    [(funVA b env)
...
;(evalA (translate
;          (parse `((lambda (x) (+ x 1)) (- 3 (let y 1 y))))
;          '())
;         '())
...
(module+ test
  ...
  (test (run `{let x 1
        ...
        (runA `{let x 1
                 {+ x {let x {+ x 1}
                        {* x 3}}}}))
  (test (run `{{lambda {x} {+ x 1}} 2})
        (runA `{{lambda {x} {+ x 1}} 2}))
  (test (run `{let f {lambda {x} {+ x 1}} {f 3}})
        (runA `{let f {lambda {x} {+ x 1}} {f 3}})))