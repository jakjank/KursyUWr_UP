...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b)] ;<------------------------------------------------HERE!
    [(appE e1 e2)
     (apply (eval e1 env) (eval e2 env) env)])) ;<-----------------HERE!

(define (apply [v1 : Value] [v2 : Value] [env : Env]) : Value ;<---HERE!
  (type-case Value v1
    [(funV x b)
...
;; A może każdy let to letrec i silnię w takim języku można napisać tak:
...
;; HUH, nie potrzebujemy letrec, to wyżej działa ślicznie. Dlaczego?
;; Khem,khem. Otóż, nie evalujemy od razu lambda... tylko wrzucamy funV do środowiska.
;; Potem podczas obliczania fact 5 dopiero evalujemy środek lambda... ale ze środowiskiem,
;; które już ma fact zdefiniowany! Zatem wszystko cudnie działa.
