...
(define (sim-wait! simulation time)
  (define (slow-incr-time simulation final-time)
    (if (= (sim-time simulation) final-time)
        (execute-actions simulation)
        (begin
          (execute-actions simulation)
          (set-sim-time! simulation (+ 1 (sim-time simulation)))
          (slow-incr-time simulation final-time))))
  (slow-incr-time simulation (+ (sim-time simulation) time)))
    
(define (execute-actions simulation)
  (if (= (action-time (heap-min (sim-act-h simulation))) (sim-time simulation))
      (begin
        ((action-function (heap-min (sim-act-h simulation))))
        (heap-remove-min! (sim-act-h simulation))
        (execute-actions simulation))
      (void)))
